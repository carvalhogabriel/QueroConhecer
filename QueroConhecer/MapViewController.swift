...
class MapViewController: UIViewController {
    ...
    var buttonUserLocation: MKUserTrackingButton!
        ...
        if CLLocationManager.locationServicesEnabled() {
            ...
            switch CLLocationManager.authorizationStatus() {
                case .authorizedAlways, .authorizedWhenInUse:
                    mapView.addSubview(buttonUserLocation)
                case .denied:
                    showMessage(type: .authorizationWarning)
                case .notDetermined:
    ...
    func configureLocationButton() {
        buttonUserLocation = MKUserTrackingButton(mapView: mapView)
        buttonUserLocation.backgroundColor  = .white
        buttonUserLocation.frame.origin.x = 10
        buttonUserLocation.frame.origin.y = 10
        buttonUserLocation.layer.cornerRadius = 5
        buttonUserLocation.layer.borderWidth = 1
        buttonUserLocation.layer.borderColor = UIColor(named: "main")?.cgColor
        
    }
    
    func showInfo() {
        labelName.text = selectedAnnotation!.title
        labelAddress.text = selectedAnnotation!.address
